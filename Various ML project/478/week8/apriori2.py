    ...
    for transaction in dataSet:
        for item in transaction:
            if not [item] in C1:
                C1.append([item])
    C1.sort()
    C1map, C1map_iter = tee(map(frozenset,C1))
    return list(C1map_iter)

def scanD(D, Ck, minSupport):
    ssCnt = {}
    D, D_iter = tee(map(set, D))
    test = list(D_iter)
    for tid in D:
        for can in Ck:
            if can.issubset(tid):
