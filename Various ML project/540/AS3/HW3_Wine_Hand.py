#SciKit DSC540 HW1
"""
Created on Thu Oct 17 19:51:38 2019
...
file1= csv.reader(open('/Users/appobs/Desktop/hw/540/week5/HW3/WineQuality_Red.csv'), delimiter=',', quotechar='"')
...
if binning==0 and cross_val==1:
    ...
    print("GradientBoosting RMSE:: %0.2f (+/- %0.2f)" % ((scores_RMSE.mean()), (scores_RMSE.std() * 2)))
    print("GradientBoosting Expl Var: %0.2f (+/- %0.2f)" % ((scores_Expl_Var.mean()), (scores_Expl_Var.std() * 2)))
    ...
    scores_RMSE = np.asarray([math.sqrt(-x) for x in scores['test_Neg_MSE']])                                       #Turns negative MSE scores into RMSE
    scores_Expl_Var = scores['test_expl_var']
    print("MLPClassifier RMSE:: %0.2f (+/- %0.2f)" % ((scores_RMSE.mean()), (scores_RMSE.std() * 2)))
    print("MLPClassifier Expl Var: %0.2f (+/- %0.2f)" % ((scores_Expl_Var.mean()), (scores_Expl_Var.std() * 2)))
    print("CV Runtime:", time.time()-start_ts)
