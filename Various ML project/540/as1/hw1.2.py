...
"""
# -*- coding: utf-8 -*-
Created on Tue Sep 24 15:23:54 2019
...
file = csv.reader(open('/Users/appobs/Desktop/hw/540/week2/HW1/WineQuality_Red.csv'), delimiter = ',', quotechar='"')
...
data_np = np.asarray(target)
target_np = np.asarray(data)
...
if lv_filter==1:
    ...
    fit_model=sel.fit(data_np)
    fitted = sel.transform(data_np)
    sel_idx=fit_mod.get_support()
    
    #Get list of selected and non-selected features (numbers and indexes)
    #idx is what we want to add, del meaning delete
    temp=[]
    temp_idx=[]
    temp_del=[]
...
####Regressors####
if binning ==0 & cross_val==0:
    #SciKit Decision Tree Regressor
    rgr = DecisionTreeRegressor(criterion='mse', splitter='best', max_depth=None, min_samples_split=3, 
                                min_samples_leaf=1, max_features=None, random_state=rand_st)
...
if binning ==0 & cross_val==1:
    ...
    start_ts=time.time()
    rgr = DecisionTreeRegressor(criterion='mse', splitter='best', max_depth=None, min_samples_split=3, 
                                mean_samples_leaf=1, max_features=None, random_state=rand_st)
